...
`scripts/shoot.sh <name>` builds with the deploy basePath, serves `out/` under `/sentinel-site`, and writes `.verify/<name>-{desktop,mobile}.png`. **Read the PNGs.** `MOTION=1 scripts/shoot.sh x` captures the animated path (under virtual time the transition may freeze mid-way or fully settle; a shot that differs from the reduced-motion one proves the JS path ran — it is not the beauty shot).
